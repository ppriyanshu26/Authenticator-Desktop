...
def show_delete_confirmation_screen(root, cred, otp_entries, build_main_ui_callback):
    ...
    ctk.CTkLabel(name_frame, text=truncated_name, font=("Segoe UI", 16, "bold"), text_color="#ffaa00").pack(anchor="w", padx=15, pady=(0, 5))
    ctk.CTkLabel(name_frame, text=f"Username: {truncated_username}", font=("Segoe UI", 12), text_color="#cccccc").pack(anchor="w", padx=15, pady=(0, 10))
    ...
    def cancel_delete():
        build_main_ui_callback(root, otp_entries)
    delete_btn = ctk.CTkButton(button_frame, text="🗑️  Delete", height=55, width=180,  command=perform_delete,  fg_color="#d32f2f", text_color="white", hover_color="#b71c1c",  font=("Segoe UI", 14, "bold"), corner_radius=10)
    delete_btn.pack(side="left", padx=15)
    cancel_btn = ctk.CTkButton(button_frame, text="✖️  Cancel", height=55, width=180, command=cancel_delete, fg_color="#3d3d3d", text_color="white", hover_color="#4d4d4d", font=("Segoe UI", 14, "bold"), corner_radius=10)
